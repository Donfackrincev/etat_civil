                ...
                <div class="page-title-container">
                    <div class="row">
                        <div class="col-12 col-md-7">
                            <h1 class="mb-0 pb-0 display-4" id="title">Liste des Temoin</h1>
                        ...
                        <table id="datatableRowsServerSide" class="data-table nowrap hover">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-muted text-small text-uppercase">Num declaration</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ ($temoinage->personne->nom)}}</td>
                                    <td>{{ ($temoinage->personne->prenom)}}</td>
                                    <td>{{ ($temoinage->personne->cni)}}</td>
                                    <td>{{ ($temoinage->personne->sexe)}}</td>
                                    <td>{{ ($temoinage->personne->profession)}}</td>
                                    <td>{{ ($temoinage->personne->date_naissance)}}</td>

                                    <td>

                                        <form method="POST" action="{{ url('/temoinage/delete/' . $temoinage->id)}}">
                                            {{method_field('DELETE')}}
                                            @csrf

                                            @if(Auth::User()->hasPermission('temoinages-show'))
                                            <a href="{{url('temoinage/' .$temoinage->id)}}">
                                                <button class="btn btn-info mb-1" type="button"> <i class="fa fa-eye"></i></button>
                                            </a>
                                            @endif

                                            @if(Auth::User()->hasPermission('temoinages-update'))
                                            <a href="{{url('temoinage/' .$temoinage->id. '/edit')}}">
                                                <button type="button" class="btn btn-secondary mb-1"> <i class="fa fa-pencil-square-o"></i> </button>
                                            </a>
                                            @endif

                                             @if(Auth::User()->hasPermission('temoinages-delete'))
